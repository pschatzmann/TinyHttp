...
class HttpServer {
    public:
        ...
        ~HttpServer(){
            Log.log(Info,"~HttpServer");
            delete []buffer;
            handler_vector.clear(false);
            request_header.clear(false);
            reply_header.clear(false);
            rewrite_vector.clear(false);
        }
        ...
        void on(char* url, MethodID method, const char* mime, const char* result) {
        ...
        void on(char*url, MethodID method, Url &redirect){
